<template>
  <div class="manor-my-collections" :class="{ 'night-mode': isDarkMode }">
    <!-- 庄园背景 -->
    <ManorBackground :isDarkMode="isDarkMode" />

    <!-- 顶部导航 -->
    <ManorNav title="我的收藏" :isDarkMode="isDarkMode" @toggle-theme="toggleTheme">
      <template #left>
    ...
    <div class="stats-section">
      <div class="stats-card">
        ...
        <div class="stat-info">
          ...
          <span class="stat-label">已收藏内容</span>
        </div>
      </div>
    </div>
        ...
        >
          ...
          <div class="card-content">
            <div class="card-header">
              <span class="post-category" v-if="item.categoryName">
                <i class="bi bi-folder-fill"></i>
                {{ item.categoryName }}
              </span>
              <span class="collect-badge">
                <i class="bi bi-bookmark-heart-fill"></i>
              </span>
            </div>
            <h3 class="post-title">{{ item.postTitle }}</h3>
            <p class="post-summary">{{ item.postSummary }}</p>
            <div class="post-meta">
              ...
              <span class="meta-item time">
                ...
                收藏于 {{ formatTime(item.collectTime) }}
              </span>
            </div>
          </div>
        </div>
    ...
    <!-- 悬浮按钮 -->
    <button class="fab-btn" @click="goToHome" title="去发现">
      <i class="bi bi-compass"></i>
    </button>
  </div>
</template>
...
async function loadCollections() {
  ...
  loading.value = true
  try {
    const res = await get(`/users/${userId.value}/favorites?page=${currentPage.value}&size=10`)
    if (res.code === 200) {
      const pageData = res.data
      if (currentPage.value === 1) {
        collections.value = pageData.list || []
      } else {
        collections.value.push(...(pageData.list || []))
      }
      hasMore.value = collections.value.length < (pageData.total || 0)
    }
  } catch (error) {
...
.night-mode .collection-card:hover {
...
.card-glow {
  position: absolute;
  inset: 0;
  background: linear-gradient(135deg, rgba(232, 168, 124, 0.05) 0%, transparent 50%);
  opacity: 0;
